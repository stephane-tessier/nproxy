...
function _respond(req, res, respondObj, next) {
  ...
  if (httpRxg.test(responder) && respondObj.cache && respondObj.cacheResponder) {
    var newURL = responder;
    responder = fixResponder(url, pattern, respondObj.cacheResponder);
    log.debug('after cache fix responder: ' + responder);
    url = newURL;
  }
